...
const FormCreateUser: FC<IProps> = (props) => {
  ...
  // danh sách admin, connie, ngọc phương
  const lockedUsers = [4, 54, 47];
  const checkButton =
    id && patient?.patient?.status === "ĐÃ ĐẾN"
      ? lockedUsers.includes(userId ?? -1)
        ? false
        : true
      : false;
                ...
                <>
                  {fields.map((field, index) => (
                    <Form.Item
                      {...formItemLayout}
                      label={`${
                        index === 0 ? t("DSDangKyHen:muc_dieu_tri") : ""
                      } `}
                      required={false}
                      key={field.key}
